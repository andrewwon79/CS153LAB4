...
80106d30:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80106d37:	00 
80106d38:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80106d3f:	00 
80106d40:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
	   uint virtualaddress=PGROUNDUP(curproc->sz);
80106d43:	8d b0 ff 0f 00 00    	lea    0xfff(%eax),%esi
	   mappages(curproc->pgdir,(void*)virtualaddress,PGSIZE,V2P(shm_table.shm_pages[i].frame),PTE_W|PTE_U);
80106d49:	8b 43 04             	mov    0x4(%ebx),%eax
	   uint virtualaddress=PGROUNDUP(curproc->sz);
80106d4c:	81 e6 00 f0 ff ff    	and    $0xfffff000,%esi
	   mappages(curproc->pgdir,(void*)virtualaddress,PGSIZE,V2P(shm_table.shm_pages[i].frame),PTE_W|PTE_U);
80106d52:	89 74 24 04          	mov    %esi,0x4(%esp)
80106d56:	05 00 00 00 80       	add    $0x80000000,%eax
80106d5b:	89 44 24 0c          	mov    %eax,0xc(%esp)
80106d5f:	8b 41 04             	mov    0x4(%ecx),%eax
80106d62:	89 04 24             	mov    %eax,(%esp)
80106d65:	e8 e6 f7 ff ff       	call   80106550 <mappages>
...
80106d6d:	8b 45 0c             	mov    0xc(%ebp),%eax
	   shm_table.shm_pages[i].refcnt++;
80106d70:	83 43 08 01          	addl   $0x1,0x8(%ebx)
80106d74:	83 c3 0c             	add    $0xc,%ebx
	   curproc->sz+=PGSIZE;
80106d77:	81 01 00 10 00 00    	addl   $0x1000,(%ecx)
   for(i=0; i<64; i++)
80106d7d:	81 fb f4 57 11 80    	cmp    $0x801157f4,%ebx
	   *pointer=(char*)virtualaddress;
80106d83:	89 30                	mov    %esi,(%eax)
	   tablefound=1;
80106d85:	b8 01 00 00 00       	mov    $0x1,%eax
   for(i=0; i<64; i++)
80106d8a:	75 97                	jne    80106d23 <shm_open+0x33>
...
80106d92:	bb f4 54 11 80       	mov    $0x801154f4,%ebx
80106d97:	74 16                	je     80106daf <shm_open+0xbf>
80106d99:	e9 aa 00 00 00       	jmp    80106e48 <shm_open+0x158>
80106d9e:	66 90                	xchg   %ax,%ax
80106da0:	83 c3 0c             	add    $0xc,%ebx
   {
   	for(i=0;i<64;i++)
80106da3:	81 fb f4 57 11 80    	cmp    $0x801157f4,%ebx
80106da9:	0f 84 99 00 00 00    	je     80106e48 <shm_open+0x158>
   	{
		if(shm_table.shm_pages[i].id==0)	//An empty page has been found! or unallocated
80106daf:	8b 03                	mov    (%ebx),%eax
80106db1:	85 c0                	test   %eax,%eax
80106db3:	75 eb                	jne    80106da0 <shm_open+0xb0>
		{
	   	   shm_table.shm_pages[i].id=id;		//Set id of unallocated to our new one
80106db5:	89 3b                	mov    %edi,(%ebx)
           	   shm_table.shm_pages[i].frame=kalloc();	//kalloc is a function that allocates 4096 byte page, or one page
80106db7:	e8 f4 b6 ff ff       	call   801024b0 <kalloc>
           	   shm_table.shm_pages[i].refcnt=1;	//This has been used once!
           	   memset(shm_table.shm_pages[i].frame,0,PGSIZE);
80106dbc:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80106dc3:	00 
80106dc4:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106dcb:	00 
           	   shm_table.shm_pages[i].refcnt=1;	//This has been used once!
80106dcc:	c7 43 08 01 00 00 00 	movl   $0x1,0x8(%ebx)
           	   memset(shm_table.shm_pages[i].frame,0,PGSIZE);
80106dd3:	89 04 24             	mov    %eax,(%esp)
           	   shm_table.shm_pages[i].frame=kalloc();	//kalloc is a function that allocates 4096 byte page, or one page
80106dd6:	89 43 04             	mov    %eax,0x4(%ebx)
           	   memset(shm_table.shm_pages[i].frame,0,PGSIZE);
80106dd9:	e8 b2 d4 ff ff       	call   80104290 <memset>
		   struct proc* curproc=myproc();
80106dde:	e8 cd c8 ff ff       	call   801036b0 <myproc>
80106de3:	89 c1                	mov    %eax,%ecx
           	   uint virtualaddress=PGROUNDUP(curproc->sz);
80106de5:	8b 00                	mov    (%eax),%eax
           	   mappages(curproc->pgdir,(void*)virtualaddress,PGSIZE,V2P(shm_table.shm_pages[i].frame),PTE_W|PTE_U);
80106de7:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80106dee:	00 
80106def:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80106df6:	00 
80106df7:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
           	   uint virtualaddress=PGROUNDUP(curproc->sz);
80106dfa:	8d b0 ff 0f 00 00    	lea    0xfff(%eax),%esi
           	   mappages(curproc->pgdir,(void*)virtualaddress,PGSIZE,V2P(shm_table.shm_pages[i].frame),PTE_W|PTE_U);
80106e00:	8b 43 04             	mov    0x4(%ebx),%eax
           	   uint virtualaddress=PGROUNDUP(curproc->sz);
80106e03:	81 e6 00 f0 ff ff    	and    $0xfffff000,%esi
           	   mappages(curproc->pgdir,(void*)virtualaddress,PGSIZE,V2P(shm_table.shm_pages[i].frame),PTE_W|PTE_U);
80106e09:	89 74 24 04          	mov    %esi,0x4(%esp)
80106e0d:	05 00 00 00 80       	add    $0x80000000,%eax
80106e12:	89 44 24 0c          	mov    %eax,0xc(%esp)
80106e16:	8b 41 04             	mov    0x4(%ecx),%eax
80106e19:	89 04 24             	mov    %eax,(%esp)
80106e1c:	e8 2f f7 ff ff       	call   80106550 <mappages>
           	   shm_table.shm_pages[i].refcnt++;
           	   curproc->sz+=PGSIZE;
...
80106e24:	8b 45 0c             	mov    0xc(%ebp),%eax
           	   shm_table.shm_pages[i].refcnt++;
80106e27:	83 43 08 01          	addl   $0x1,0x8(%ebx)
80106e2b:	83 c3 0c             	add    $0xc,%ebx
           	   curproc->sz+=PGSIZE;
80106e2e:	81 01 00 10 00 00    	addl   $0x1000,(%ecx)
   	for(i=0;i<64;i++)
80106e34:	81 fb f4 57 11 80    	cmp    $0x801157f4,%ebx
		   *pointer=(char*)virtualaddress;
80106e3a:	89 30                	mov    %esi,(%eax)
   	for(i=0;i<64;i++)
80106e3c:	0f 85 6d ff ff ff    	jne    80106daf <shm_open+0xbf>
80106e42:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
		}
   	}
   }
   release(&(shm_table.lock));
80106e48:	c7 04 24 c0 54 11 80 	movl   $0x801154c0,(%esp)
80106e4f:	e8 ec d3 ff ff       	call   80104240 <release>
   return 0; //added to remove compiler warning -- you should decide what to return
}
80106e54:	83 c4 2c             	add    $0x2c,%esp
...
80106e7b:	eb 28                	jmp    80106ea5 <shm_close+0x45>
80106e7d:	8d 76 00             	lea    0x0(%esi),%esi
80106e80:	8b 50 08             	mov    0x8(%eax),%edx
   {
	if(shm_table.shm_pages[i].id==id)
        {
	   shm_table.shm_pages[i].refcnt--;
...
80106e83:	85 d2                	test   %edx,%edx
80106e85:	75 14                	jne    80106e9b <shm_close+0x3b>
	{
	   shm_table.shm_pages[i].id=0;
80106e87:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
	   shm_table.shm_pages[i].frame=0;
80106e8d:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
	   shm_table.shm_pages[i].refcnt=0;
80106e94:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
80106e9b:	83 c0 0c             	add    $0xc,%eax
   for(i=0;i<64;i++)
80106e9e:	3d f4 57 11 80       	cmp    $0x801157f4,%eax
80106ea3:	74 13                	je     80106eb8 <shm_close+0x58>
	if(shm_table.shm_pages[i].id==id)
80106ea5:	3b 18                	cmp    (%eax),%ebx
80106ea7:	75 d7                	jne    80106e80 <shm_close+0x20>
	   shm_table.shm_pages[i].refcnt--;
80106ea9:	8b 48 08             	mov    0x8(%eax),%ecx
80106eac:	8d 51 ff             	lea    -0x1(%ecx),%edx
80106eaf:	89 50 08             	mov    %edx,0x8(%eax)
80106eb2:	eb cf                	jmp    80106e83 <shm_close+0x23>
80106eb4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
	}
    }   
    release(&(shm_table.lock));
80106eb8:	c7 04 24 c0 54 11 80 	movl   $0x801154c0,(%esp)
80106ebf:	e8 7c d3 ff ff       	call   80104240 <release>
return 0; //added to remove compiler warning -- you should decide what to return
}
80106ec4:	83 c4 14             	add    $0x14,%esp
